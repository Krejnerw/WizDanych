...
print(seria[(seria > 1)])
print((seria > 10))#porownac z powyzszym bo jest jakas roznica
...
print(df[df['Populacja']>1200000000])#populacja -kolumna
...
seria['Wiesiek'] = 15
print(seria.Wiesiek)
seria['Alan'] = 16#jak nie ma zostanie stworzony
print(seria)
...
df.loc[5] = ['Polska', 'Warszawa', 38675467]#loc odnosi sie do wiierszy
print(df)
# dzialania na orginalnych danych 
...
print(df)#musi liczyt tyle samo ile l wierszy

# Pandas ma również własne funkcje sortowania danych
print(df.sort_values(by='Kraj'))#cos tu chyba nie ok
print(df.sort_values(by=['Kraj','Kontynent'],ascending=False))#cos tu chyba nie ok

# grupowania
grouped = df.groupby(['Kontynent'])
print('to tu ',grouped.get_group('Europa'))
